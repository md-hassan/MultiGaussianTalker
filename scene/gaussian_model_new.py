...
from scene.deformation import canonical_network
...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, args):
        ...
        self._canonical = canonical_network(args) # for creating the 3DGS
        ...
        self._canonical_table = torch.empty(0)    # earlier called _deformation_table: needed for pruning
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, time_line: int):
        ...
        self._canonical = self._canonical.to("cuda") 
        ...
        self._canonical_table = torch.gt(torch.ones((self.get_xyz.shape[0]),device="cuda"),0)
    def training_setup(self, training_args):
        
        self.percent_dense = training_args.percent_dense
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self._canonical_accum = torch.zeros((self.get_xyz.shape[0],3),device="cuda")
        ...
        if "canonical" in training_args.train_l:
            train_l.append({'params': list(self._canonical.get_mlp_parameters()), 'lr': training_args.deformation_lr_init * self.spatial_lr_scale, "name": "canonical"})
        ...
        # !!! NEED TO CHECK WHAT THIS GRID IS (is it only for canonical part?)
        if "grid" in training_args.train_l:
            train_l.append({'params': list(self._canonical.get_grid_parameters()), 'lr': training_args.grid_lr_init * self.spatial_lr_scale, "name": "canonical_grid"})
        if "grid" in training_args.train_l:
            train_l.append({'params': list(self._deformation.get_grid_parameters()), 'lr': training_args.grid_lr_init * self.spatial_lr_scale, "name": "deformation_grid"})
        ...
        self.canonical_scheduler_args = get_expon_lr_func(lr_init=training_args.deformation_lr_init*self.spatial_lr_scale,
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if  "grid" in param_group["name"]:
                ...
                param_group['lr'] = lr
                # return lr
            elif param_group["name"] == "canonical":
                lr = self.canonical_scheduler_args(iteration)
                param_group['lr'] = lr
                # return lr
    ...
    def load_model(self, path):
        ...
        weight_dict = torch.load(os.path.join(path,"canonical.pth"),map_location="cuda")
        self._canonical.load_state_dict(weight_dict)
        self._canonical = self._canonical.to("cuda")
        ...
        self._canonical_table = torch.gt(torch.ones((self.get_xyz.shape[0]),device="cuda"),0)
        self._canonical_accum = torch.zeros((self.get_xyz.shape[0],3),device="cuda")
        if os.path.exists(os.path.join(path, "canonical_table.pth")):
            self._canonical_table = torch.load(os.path.join(path, "canonical_table.pth"),map_location="cuda")
        if os.path.exists(os.path.join(path, "canonical_accum.pth")):
            self._canonical_accum = torch.load(os.path.join(path, "canonical_accum.pth"),map_location="cuda")

        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
        
    def save_deformation(self, path):
        torch.save(self._canonical.state_dict(),os.path.join(path, "_canonical.pth"))
        torch.save(self._deformation.state_dict(),os.path.join(path, "deformation.pth"))
        torch.save(self._canonical_table,os.path.join(path, "canonical_table.pth"))
        torch.save(self._canonical_accum,os.path.join(path, "canonical_accum.pth"))
    ...
    def prune_points(self, mask):
        ...
        self._canonical_accum = self._canonical_accum[valid_points_mask]
        self.xyz_gradient_accum = self.xyz_gradient_accum[valid_points_mask]
        self._canonical_table = self._canonical_table[valid_points_mask]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_canonical_table):
        d = {"xyz": new_xyz,
        ...
        self._canonical_table = torch.cat([self._canonical_table,new_canonical_table],-1)
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self._canonical_accum = torch.zeros((self.get_xyz.shape[0], 3), device="cuda")
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_canonical_table = self._canonical_table[selected_pts_mask].repeat(N)
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_canonical_table)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, density_threshold=20, displacement_scale=20, model_path=None, iteration=None, stage=None):
        ...
        new_canonical_table = self._canonical_table[selected_pts_mask]
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_canonical_table)

    @property
    def get_aabb(self):
        # return self._deformation.get_aabb
        return self._canonical.get_aabb
    ...
    def add_point_by_mask(self, selected_pts_mask, perturb=0):
        ...
        new_canonical_table = self._canonical_table[selected_pts_mask][mask]
    ...
    def standard_constaint(self):
        ...
        means3D_canonical, scales_canonical, rotations_canonical, _ = self._canonical(means3D, scales, rotations, opacity, time)
        position_error = (means3D_canonical - means3D)**2
        rotation_error = (rotations_canonical - rotations)**2 
        scaling_erorr = (scales_canonical - scales)**2
    ...
    @torch.no_grad()
    def update_canonical_table(self,threshold):
        self._canonical_table = torch.gt(self._canonical_accum.max(dim=-1).values/100,threshold)
    def print_deformation_weight_grad(self):
        for name, weight in self._canonical.named_parameters():
    ...
    def _plane_regulation(self):
        multi_res_grids = self._canonical.canonical_net.grid.grids
    ...
    def save_feature_mlps(self, path):
        ...
        self._canonical.save_feature_mlps(path)